...
plt.text(10, np.max(full_mass_list)*0.9, 'Brown&Francis: $m=0.0185\cdot D^{1.9}$', bbox=dict(facecolor='red', alpha=0.2))
plt.text(10, np.max(full_mass_list)*0.95,  'Power Law Fit: $m = {0:.4f}\cdot D^{{{1:.3f}}}$\nn = {2}, unique: {3}'.format(amp/1000, index, n, nu),
         bbox=dict(facecolor='blue', alpha=0.2))

plt.text(10, np.max(full_mass_list) * 0.8,
...
plt.plot(dims_spaced, powerlaw(dims_spaced, amp, index))
